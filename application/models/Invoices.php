     ...
     public function getInvoiceList($postData=null){
       $this->load->library('occational');
        ...
        // $shippingMethod = $data1->row()->shipping_method;
         ...
         $this->db->select("a.*,b.customer_name,u.first_name,u.last_name,b.customer_mobile,b.customer_address,b.district,b.area,b.areaid,b.division");
         $this->db->from('invoice a');
         $this->db->join('customer_information b', 'b.customer_id = a.customer_id','left');
         $this->db->join('users u', 'u.user_id = a.sales_by','left');
         if($usertype == 2){
 ...
 $bankc = array(
   ...
   $config_data = $this->db->select('*')->from('sms_settings')->get()->row();
    ...
    public function retrieve_invoice_editdata($invoice_id) {
        $this->db->select('a.*, sum(c.quantity) as sum_quantity, a.total_tax as taxs,a. prevous_due,b.customer_mobile,b.customer_name as customer_name,c.*,c.tax as total_tax,c.product_id,d.product_name,d.product_model,d.tax,d.unit,d.*');
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
